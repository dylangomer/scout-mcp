...
class TestScoutFindShape:
    ...
    async def test_scout_find_shape(self):
        ...
        with (
            ...
            patch.object(ranker, "rank_servers", AsyncMock(return_value=MOCK_RANKED_RESULTS)),
        ):
    ...
    async def test_scout_find_sorted_by_score(self):
        ...
        # rank_servers returns [90, 80, 70, 50] — verify the order is preserved
        with (
            patch.object(server, "search_registry", AsyncMock(return_value=MOCK_REGISTRY_SERVERS)),
            patch.object(ranker, "rank_servers", AsyncMock(return_value=MOCK_RANKED_RESULTS)),
        ):
    ...
    async def test_scout_find_default_max_results(self):
        ...
        # Provide 6 ranked results to verify the cap applies
        six_ranked = [
        ...
        with (
            patch.object(server, "search_registry", AsyncMock(return_value=MOCK_REGISTRY_SERVERS)),
            patch.object(ranker, "rank_servers", AsyncMock(return_value=six_ranked)),
        ):
    ...
    async def test_scout_find_filters_stdio(self):
        ...
        # Capture what list is passed to rank_servers
        captured_servers = []
        ...
        with (
            patch.object(server, "search_registry", AsyncMock(return_value=MOCK_REGISTRY_SERVERS)),
            patch.object(ranker, "rank_servers", capturing_rank),
        ):
            results = await server.scout_find("any context")

        # The list passed to rank_servers must contain no stdio servers
        for s in captured_servers:
            assert s["remote_url"] is not None, (
                f"Server {s['name']} with remote_url=None passed to rank_servers"
            )

        # And final results also must not contain any stdio server
        for r in results:
...
class TestRegistryRetry:
    ...
    async def test_registry_retry_on_first_failure(self):
        """When first registry call raises HTTPError, second succeeds — scout_find returns results."""
        # Build a minimal successful registry JSON response
        registry_json = {
            "servers": [
                {
                    "server": {
        ...
        # First call raises HTTPStatusError, second returns success
        call_count = 0
        ...
        with patch("httpx.AsyncClient", return_value=mock_client_instance):
            # Calling search_registry directly to test retry
    ...
    async def test_registry_retry_exhausted(self):
        """When both registry attempts fail, RuntimeError is raised with 'Registry unreachable'."""
        mock_client_instance = AsyncMock()
